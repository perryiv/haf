...
Result::~Result()
{
  Usul::Functions::noThrow 
    ( boost::bind ( &Result::_destroy, this ), "4030095801" );
}
...
Result &Result::operator >> ( float &value )
{
  ...
  *this >> temp;
  ...
  {
    throw Usul::Exceptions::Error ( "2911940382", Usul::Strings::format (
      sizeof ( double ), " byte float ", temp, " will not fit in a ", 
      sizeof ( float ), " byte float" ) );
  }
...
Result &Result::operator >> ( unsigned int &value )
{
  ...
  *this >> temp;
  if ( temp < 0 )
  {
    throw Usul::Exceptions::Error ( "3361492869", Usul::Strings::format (
      sizeof ( int ), "-byte integer ", temp, " will not convert to ", 
      sizeof ( unsigned int ), "-byte unsigned integer" ) );
  }
...
Result &Result::operator >> ( Usul::Types::UInt64 &value )
{
  ...
  *this >> temp;
  ...
  {
    throw Usul::Exceptions::Error ( "3361492869", Usul::Strings::format (
      sizeof ( Usul::Types::Int64 ), "-byte integer ", temp, 
      " will not convert to ", 
      sizeof ( Usul::Types::UInt64 ), "-byte unsigned integer" ) );
  }
