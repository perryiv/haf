...
void Execute::_processCommandLine()
{
  ...
  typedef Arguments::Strings Args;
  typedef std::vector<std::string> StringList;
  Args args ( Arguments::instance().get() );
...
void Execute::_checkInput()
{
  ...
  {
    throw Usul::Exceptions::Error ( "4187684402", "No database URL given" );
  }
  ...
  if ( true == user.empty() )
  {
    throw Usul::Exceptions::Error ( "3764694510", "No database user given" );
  }
  ...
  {
    throw Usul::Exceptions::Error ( "3924495829", "No database password given" );
  }
}
...
void Execute::_executeCommands()
{
  ...
  // Make the connection.
  Connection::RefPtr connection ( Connection::connect
  ...
  // Need local scope because the connection has to live longer than the roll-back object.
  {
    ...
    Usul::Scope::Caller::RefPtr rollback ( Usul::Scope::makeCaller 
      ( boost::bind ( &Connection::rollback, connection.get() ) ) );
...
void Execute::_executeCommands ( Connection::RefPtr connection, std::ostream &out )
{
  ...
  if ( false == connection.valid() )
    throw Usul::Exceptions::Error ( "4281592813", "Invalid connection given" );
...
Execute::Result::RefPtr Execute::_executeCommand ( Connection::RefPtr connection, const std::string &sql )
{
  // Check input.
  if ( false == connection.valid() )
    throw Usul::Exceptions::Error ( "4640977390", "Invalid connection given" );
...
{
  std::ostringstream s;
  s << Usul::CommandLine::Arguments::instance().get ( 0 ) << ' ';
