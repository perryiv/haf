...
Connection::Connection ( oracle::occi::Connection *c ) : BaseClass(),
  _connection ( c ),
  _executing ( 0x0 )
{
}
...
Connection::~Connection()
{
  Usul::Functions::noThrow ( boost::bind 
    ( &Connection::_destroy, this ), "9116646720" );
}
  ...
  const std::string &password )
{
  ...
  USUL_TRY_BLOCK
  {
    oracle::occi::Connection *c
      ( Environment::instance().env()->createConnection
        ( user, password, database ) );

    if ( 0x0 != c )
    {
      return RefPtr ( new Connection ( c ) );
    }
  }
  catch ( const oracle::occi::SQLException &e )
  {
    message = ( e.what() ? e.what() : "" );
  }

  throw Usul::Exceptions::Error ( "2964914308", Usul::Strings::format (
    "Failed to connect to database '", database, 
    "' with user '", user, "'. ", message ) );
}
  ...
  if ( 0x0 != _connection )
  {
    ...
    catch ( const oracle::occi::SQLException &e )
    {
      std::cout << Usul::Strings::format ( 
        "Error 1403031671: Failed to terminate connection. ", 
        ( ( 0x0 != e.what() ) ? e.what() : "" ), '\n' ) << std::flush;
    }
    USUL_DEFINE_CATCH_BLOCKS ( "2924598246" )
  }
}
...
Result *Connection::_execute ( oracle::occi::Connection *c, oracle::occi::Statement *s, bool select )
{
  ...
  USUL_TRY_BLOCK
  {
    ...
    Usul::Scope::Caller::RefPtr reset ( Usul::Scope::makeCaller (
      boost::bind ( 
        &Connection::_setExecutingStatement, this, 
        static_cast < oracle::occi::Statement * > ( 0x0 ) ) ) );
...
void Connection::sendCancelRequestToServer()
{
  ...
  Usul::Functions::noThrow ( boost::bind 
    ( &::OCIBreak, server, errorCallback ), "2257760186" );
  Usul::Functions::noThrow ( boost::bind 
    ( &::OCIReset, server, errorCallback ), "3100516377" );
}
