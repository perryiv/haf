...
#ifndef _MINERVA_DOCUMENT_EXTENTS_CLASS_H_
#define _MINERVA_DOCUMENT_EXTENTS_CLASS_H_
...
inline void ThisClass::split ( Extents &ll, Extents &lr, Extents &ul, Extents &ur ) const
{
  const Vertex &mn ( this->minimum() );
  const Vertex &mx ( this->maximum() );
  const Vertex md ( ( mx + mn ) * static_cast < ValueType > ( 0.5 ) );

  ll = Extents ( mn[0], mn[1], md[0], md[1] );
  lr = Extents ( md[0], mn[1], mx[0], md[1] );
  ul = Extents ( mn[0], md[1], md[0], mx[1] );
  ur = Extents ( md[0], md[1], mx[0], mx[1] );
}
...
#endif // _MINERVA_DOCUMENT_EXTENTS_CLASS_H_
