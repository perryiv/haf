...
public:
  ...
  USUL_REFERENCED_CLASS ( InputFile );
...
inline void read ( const std::string &file, InputFile::Buffer &buffer, bool throwIfError )
{
...
template < class FileType > 
inline void read ( const std::string &file, std::string &contents, bool throwIfError )
{
  // Initialize.
  contents.clear();
