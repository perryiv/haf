    ...
    template <> struct Type < std::vector<unsigned char>, Usul::Math::Vec2d >
    {
      ...
      enum
      {
        NUM_BYTES = ( sizeof ( unsigned char ) + 
		      sizeof ( Usul::Types::UInt32 ) + 2 * 
		      sizeof ( Usul::Types::Float64 ) ),
        POINT_TYPE = 1
      };

      static void convert ( const Blob &from, Usul::Math::Vec2d &to )
      {
        USUL_ASSERT_SAME_TYPE 
	  ( Usul::Types::Float64, Usul::Math::Vec2d::value_type );

        if ( ThisType::NUM_BYTES != from.size() )
        {
          throw std::invalid_argument 
	    ( Usul::Strings::format 
	      ( "Error 3366212750: Invalid character vector size ", 
		from.size(), ", expected ", 
		static_cast < unsigned int > ( ThisType::NUM_BYTES ) ) );
        }

        const unsigned char *endianPtr ( &from[0] );
        const unsigned char endian ( *endianPtr ); ++endianPtr;
        const Usul::Types::UInt32 *typePtr 
	  ( USUL_UNSAFE_CAST ( Usul::Types::UInt32 *, endianPtr ) );
        Usul::Types::UInt32 type ( *typePtr ); ++typePtr;
        const Usul::Types::Float64 *values 
	  ( USUL_UNSAFE_CAST ( Usul::Types::Float64 * , typePtr ) );
        ...
        if ( ThisType::POINT_TYPE != type )
        {
          throw std::invalid_argument 
	    ( Usul::Strings::format 
	      ( "Error 2344339624: Invalid geometry type ", 
		type, ", expected ", 
		static_cast < unsigned int > ( ThisType::POINT_TYPE ) ) );
        }
...
{ 
  namespace Convert
  {
    ...
    >
    {
      ...
      enum
      {
        MIN_BYTES = ( sizeof ( unsigned char ) + 
		      sizeof ( Usul::Types::UInt32 ) + 4 * 
		      sizeof ( Usul::Types::Float64 ) ),
        LINE_TYPE = 2
      };
	  ...
	  ( Usul::Types::Float64, Usul::Math::Vec2d::value_type );
        ...
        if ( ThisType::MIN_BYTES > from.size() )
        {
          throw std::invalid_argument 
	    ( Usul::Strings::format 
	      ( "Error 3438257445: Invalid character vector size ", 
		from.size(), ", expected at least ", 
		static_cast < unsigned int > ( ThisType::MIN_BYTES ) ) );
        }

        const unsigned char *endianPtr ( &from[0] );
        const unsigned char endian ( *endianPtr ); ++endianPtr;
        const Usul::Types::UInt32 *uIntPtr 
	  ( USUL_UNSAFE_CAST ( Usul::Types::UInt32 *, endianPtr ) );
        Usul::Types::UInt32 type ( *uIntPtr ); ++uIntPtr;
        Usul::Types::UInt32 count ( *uIntPtr ); ++uIntPtr;
        const Usul::Types::Float64 *values 
	  ( USUL_UNSAFE_CAST ( Usul::Types::Float64 * , uIntPtr ) );
        ...
        if ( ThisType::LINE_TYPE != type )
        {
          throw std::invalid_argument 
	    ( Usul::Strings::format
	      ( "Error 1391184162: Invalid geometry type ", 
		type, ", expected ", 
		static_cast < unsigned int > ( ThisType::LINE_TYPE ),
