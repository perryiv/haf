...
public:
  ...
  typedef Matrix44 < T, IndexType, ErrorCheckerType > ThisType;
  ...
  template < class Matrix_, typename Real_ > static void makePerspective ( 
    Real_ fovyInRadians, 
    Real_ aspect, 
    Real_ zNear, 
    Real_ zFar, 
  ...
  void makePerspective ( T fovyInRadians, T aspect, T zNear, T zFar )
  {
    makePerspective ( fovyInRadians, aspect, zNear, zFar, _m );
  }


  /////////////////////////////////////////////////////////////////////////////
  //
  //  Return a matrix like a call to glIdentity() and gluPerspective() would.
  //
  /////////////////////////////////////////////////////////////////////////////

  static ThisType perspective ( T fovyInRadians, T aspect, T zNear, T zFar )
  {
    ThisType m;
    m.makePerspective ( fovyInRadians, aspect, zNear, zFar );
    return m;
  }
  ...
  {
    ThisType m;
    if ( false == this->inverse ( m ) )
    {
      throw std::runtime_error ( "Error 2318439298: Matrix inverse does not exist" );
    }
    return m;
  }
  ...
  //
  //  http://cgit.freedesktop.org/mesa/mesa/tree/src/glu/sgi/libutil/project.c
  //
  /////////////////////////////////////////////////////////////////////////////

  template < class Matrix_, class Vector3_ > static void lookAt ( 
    const Vector3_ &eye, const Vector3_ &center, const Vector3_ &upDir, Matrix_ &m )
  {
    typedef Vector3_ V;
    typedef Matrix_ M;

    V forward ( center[0] - eye[0], center[1] - eye[1], center[2] - eye[2] );
    normalize ( forward );

    V up ( upDir[0], upDir[1], upDir[2] );

    V side ( forward.cross ( up ) );
    normalize ( side );
  ...
  /////////////////////////////////////////////////////////////////////////////
  //
  //  Return the matrix like a call to "glIdentity(); glLookAt();"
  //  eye    = your vantage point.
  //  center = the place you want to look.
  //  up     = a vector that defines "up".
  //
  ...
  template < class Matrix_, class Vector3_ > static ThisType makeLookAt ( 
    const Vector3_ &eye, const Vector3_ &center, const Vector3_ &up )
  {
    ThisType m;
    lookAt ( eye, center, up, m );
    return m;
  }
  ...
  template < class Pred > bool compare ( const Matrix44 &m, Pred p ) const
  {
    return ( 
      p ( _m[0], m[0] ) && p ( _m[4], m[4] ) && p ( _m[ 8], m[ 8] ) && p ( _m[12], m[12] ) && 
      p ( _m[1], m[1] ) && p ( _m[5], m[5] ) && p ( _m[ 9], m[ 9] ) && p ( _m[13], m[13] ) && 
      p ( _m[2], m[2] ) && p ( _m[6], m[6] ) && p ( _m[10], m[10] ) && p ( _m[14], m[14] ) && 
      p ( _m[3], m[3] ) && p ( _m[7], m[7] ) && p ( _m[11], m[11] ) && p ( _m[15], m[15] ) );
  }
  ...
  {
    return ( 
      p ( _m[0] ) && p ( _m[4] ) && p ( _m[ 8] ) && p ( _m[12] ) && 
      p ( _m[1] ) && p ( _m[5] ) && p ( _m[ 9] ) && p ( _m[13] ) && 
      p ( _m[2] ) && p ( _m[6] ) && p ( _m[10] ) && p ( _m[14] ) && 
      p ( _m[3] ) && p ( _m[7] ) && p ( _m[11] ) && p ( _m[15] ) );
  }
  ...
  /////////////////////////////////////////////////////////////////////////////
  //
  //  Return a rotation about an arbitrary axis.
  //  radians = an angle in radians
  //  axis    = a normalized vector that specifies the axis to rotate about.
  //
  /////////////////////////////////////////////////////////////////////////////

  template < class Vector3_, typename Real_ > static ThisType rotation ( Real_ radians, const Vector3_ &axis )
  {
    ThisType m;
    m.makeRotation ( radians, axis );
    return m;
  }
  ...
  /////////////////////////////////////////////////////////////////////////////
  //
  //  Return a 90-degree (or multiple thereof) rotation about arbitrary axis.
  //  Add more as needed.
  //
  /////////////////////////////////////////////////////////////////////////////

  template < int degrees > static ThisType rotation ( const Usul::Math::Vector3<T> &axis );
  template <> static ThisType rotation<90> ( const Usul::Math::Vector3<T> &axis )
  {
    ThisType m;
    m.makeRotation ( static_cast < T > ( 0 ), static_cast < T > ( 1 ), axis );
    return m;
  }
  ...
  ( const Usul::Math::Matrix44<T,I,E> &m, const Usul::Math::Vector3<T,I,E> &b )
{
  ...
  Vector4 v;
  const T one ( 1 );
  v[3] =  one / ( m[3] * b[0] + m[7] * b[1] + m[11] * b[2] + m[15] );
